...
pub struct MintConf {
    ...
    /// HIP-2 v2 soft-fork height; 0 = disabled until configured.
    pub mortgage_activation_height: u64,
    /// Global outstanding IOU cap (zhu); 0 uses protocol default.
    pub mortgage_max_outstanding_zhu: u64,
    /// Dev only: mortgage period = 10 blocks (requires hip25_testnet_seed).
    pub hip2_testnet_demo_periods: bool,
}
...
impl MintConf {
    ...
    pub fn new(ini: &IniObj) -> MintConf {
        ...
        let cnf = MintConf {
            ...
            mortgage_activation_height: ini_must_u64(&sec, "mortgage_activation_height", 0),
            mortgage_max_outstanding_zhu: ini_must_u64(&sec, "mortgage_max_outstanding_zhu", 0),
            hip2_testnet_demo_periods: ini_must_bool(&sec, "hip2_testnet_demo_periods", false),
        };

        if let Err(e) = cnf.validate_hip25_dev_flags() {
            panic!("[Config Error] {}", e);
        }
        if let Err(e) = cnf.validate_hip2_dev_flags() {
            panic!("[Config Error] {}", e);
        }
    ...
    /// Reject dev-only HIP-2 flags on non-dev chain_id (mainnet safety).
    pub fn validate_hip2_dev_flags(&self) -> Result<(), String> {
        if self.hip2_testnet_demo_periods && !self.hip25_testnet_seed {
            return Err(
                "hip2_testnet_demo_periods requires hip25_testnet_seed = true".to_string(),
            );
        }
        if self.hip2_testnet_demo_periods && self.chain_id != HIP25_DEV_CHAIN_ID {
            return Err(format!(
                "hip2_testnet_demo_periods is only allowed on dev chain_id {} (configured chain_id={})",
                HIP25_DEV_CHAIN_ID, self.chain_id
            ));
        }
        Ok(())
